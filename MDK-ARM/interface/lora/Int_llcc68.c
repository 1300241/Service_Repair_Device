...
uint8_t Int_llcc68_init(void)
{
    ...
    // 2. 初始化llcc68 校验gs_handle中的函数是否完整
...
uint8_t Int_llcc68_receive(uint8_t *buf, uint16_t *len)
{
    ...
    if (gs_handle.receive_buf_len != 0)
    {
        // 接收到了数据
        memcpy(buf, gs_handle.receive_buf, gs_handle.receive_buf_len);
        *len = gs_handle.receive_buf_len;
        return 0;
    }
    return 1;
}
